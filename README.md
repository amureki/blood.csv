...
4. Click a marker's **View N results** action for its complete dated table, followed by a small chart when at least two numeric results are available. The marker name and action share one large tap area. Opening details brings the marker to the top of the viewport, including when switching from another open history. Each history row shows the date, value, and lab, newest first, with no further disclosure level. Tap the chart, or focus it and use Left / Right arrow keys. **Close details** returns to that marker.
...
**Since previous test** compares each marker’s latest result with its result on the most recent earlier measurement date, using the same unit. The comparison shows the previous value and date on desktop; on phones, it shows the change followed by **since** and the previous date. If either date has multiple results, either value is not an exact number, or no earlier date exists, it shows the reason instead of a change. It does not skip unavailable comparisons to use an older result. Full history remains available in the details.
