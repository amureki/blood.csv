...
**Since previous test** compares each marker’s latest result with its result on the most recent earlier measurement date, using the same unit. The comparison shows the previous value and date on desktop. If either date has multiple results, either value is not an exact number, or no earlier date exists, it shows the reason instead of a change. It does not skip unavailable comparisons to use an older result. Full history remains available in the details.
